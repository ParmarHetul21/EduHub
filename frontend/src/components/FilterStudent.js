...
class FilterStudent extends Component {
  ...
  render() {
    let data = this.state.students.filter(
      (s) =>
        s.semester.toString() ===
          this.props.match.params.semester.split(":")[0] &&
        s.batch.toString() === this.props.match.params.semester.split(":")[1]
    );

    return (
      <>
        <div className="bg_color_admin" style={{ width: "100%" }}>
          <h2>{`Semester - ${this.props.match.params.semester.split(":")[0]} (${
            this.props.match.params.semester.split(":")[1]
          })`}</h2>
        </div>
        <div
